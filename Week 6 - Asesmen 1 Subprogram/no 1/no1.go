...
func volume(r, t float64) float64 {
	/* mengembalikan volume tabung yang memiliki jari-jari lingkaran r dan tinggi
	   t, yang mana volume adalah luas alas x tinggi tabung */
	return pi * (r * r) * t
}

func massa(r, t, p float64) float64 {
	/* mengembalikan massa tabung yang memiliki jari-jari lingkaran r dan tinggi
	   t, serta massa jenis p, yang mana massa = volume x massa jenis */
...
func display(m1, m2 float64) {
	/* I.S. terdefinisi massa zat cair kiri m1 dan massa zat cair kanan m2 pada timbangan
	   F.S. menampilkan "BALANCE" apabile m1 dan m2 adalah sama, atau selisih positif (nilai mutlak) apabila m1 dan m2 berbeda */
	var selisih float64
	if m1 == m2 {
		fmt.Println("BALANCE")
	} else {
		selisih = m1 - m2
		if selisih < 0 {
			selisih = -1 * selisih //kalo selisihnya negatif, ubah jadi positif (nilai mutlak)
		}
...
func main() {
	...
	var mjKiri, mjKanan float64       // massa jenis zar cair
	var massaKiri, massaKanan float64 // massa zar cair kiri dan kanan
